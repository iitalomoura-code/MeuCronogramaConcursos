...
assert.ok(open.indexOf("els.strategicAdvisorModal.hidden = false") < open.indexOf("strategicPerfYield"), "O modal precisa aparecer antes de qualquer cálculo adiado.");
...
assert.ok(open.includes('strategicPerfYield(performanceTrace, { frames: 2 })'), "A abertura precisa aguardar dois frames antes do primeiro cálculo pesado.");
...
assert.ok(content.includes("strategicAdvisorModelForModal") && content.includes("await strategicPerfYield(performanceTrace)"), "O modelo deve ser montado em uma etapa cooperativa, com pintura entre as seções.");
...
assert.ok(model.includes('"strategicPlanningTopicsForModal"') && model.includes('"strategicAdvisorModel build"'), "A instrumentação deve separar o custo dos tópicos estratégicos do build do Orientador.");
