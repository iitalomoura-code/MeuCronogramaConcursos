...
const entries = Array.from({ length: 120 }, (_, index) => ({
  materia: `Matéria ${index % 12}`,
  assunto: `Tema ${index}`,
  questoes: 10 + (index % 7),
  acertos: 5 + (index % 5),
}));
const targets = entries.map((entry) => ({ materia: entry.materia, assunto: entry.assunto }));
...
assert.equal(planningIndex.stats.topicCandidateScans, 1200, "Cada tema deve examinar somente o bucket da sua matéria, não o histórico inteiro.");
assert.equal(baselineScans, 28800, "A linha de base controlada deve revelar o custo de varrer todo o histórico a cada chamada.");
...
console.log("OK - profiling do Orientador e índice por revisão preservam resultados e eliminam scans repetidos.");
