...
assert.ok(app.includes('dataSource: "cloud"'), "O aplicativo deve iniciar com o Supabase como fonte de dados.");
assert.ok(app.includes("ACTIVE_CLOUD_PLAN_KEY"), "O \u00faltimo planejamento online deve ser lembrado separadamente.");
assert.ok(app.includes("function restoreAppState() { return false; }"), "A inicialização não pode restaurar planejamentos locais.");
...
assert.ok(!app.includes("planStorageKey"), "A aplicação não deve manter chaves locais para snapshots de planejamento.");
...
assert.ok(app.includes("function readLocalKnowledgeBase() { return null; }") && app.includes("function bootstrapKnowledgeBase"), "A base permanente deve iniciar pela nuvem, sem cache persistente no navegador.");
...
assert.ok(index.includes("js/diagnostic-confidence.js?v=20260912-ai-coach-local-smoke") && index.includes("js/ai-strategic-snapshot.js?v=20260913-ai-coach-payload-compact"), "O contrato de leitura da IA deve carregar como módulo explícito antes do aplicativo.");
