...
assert.ok(app.includes('<div class="focused-study-backdrop" aria-hidden="true"></div>'), "Clicar fora do modo focado não deve minimizar a sessão sem querer.");
assert.ok(app.includes('data-finalize-active-focused>Retomar para finalizar'), "O cartão minimizado deve explicar que o botão reabre o estudo antes de salvar.");
...
assert.ok(app.includes('measureFocusPerformance(performanceTrace, "remove overlay"') && app.includes("await yieldForInteraction();"), "Finalizar no modo foco deve fechar o painel antes dos cálculos de resultado.");
...
assert.ok(app.includes("focusedStudyPersistenceTimer = window.setTimeout(() => scheduleAutoSave({ invalidate: false })"), "Timer e minimização devem persistir sem invalidar os caches estratégicos.");
...
assert.ok(app.includes('scheduleFocusedStudyCloseWork({ silent, persistLabel: "Sessão em andamento", performanceTrace });'), "O botão de fechar deve remover o modo foco antes do autosave e do rerender de Continuar.");
assert.ok(app.includes('measureFocusPerformance(performanceTrace, "sync state"') && app.includes('measureFocusPerformance(performanceTrace, "remove overlay"'), "Fechar o modo foco deve parar o timer e retirar o overlay no mesmo ciclo do toque.");
assert.ok(app.includes("function renderActiveFocusSessionCard"), "A sessão em andamento deve ter um caminho rápido próprio.");
assert.ok(app.includes("function scheduleContinuePanelRefresh"), "A reconstrução completa de Continuar deve poder ser adiada.");
