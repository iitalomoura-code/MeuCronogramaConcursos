...
(function initErrorAnalysis(global) {
  ...
  function normalizeRecord(record = {}, index = 0) {
    ...
    return {
      ...
      automatic: Boolean(record.registroAutomatico || record.automatic || record.autoGenerated),
    };
  }
  ...
  function aggregate(records = [], interventions = [], now = Date.now()) {
    const parsedRecords = records.map(normalizeRecord).filter((record) => record.key !== "::" && record.count > 0);
    // Um resultado de sessão pode trazer a contagem total automática e registros
    // manuais que descrevem erros específicos. Para não contar os dois, a contagem
    // automática representa a sessão; os manuais preservam tipo e observação.
    const sessionGroups = new Map();
    parsedRecords.forEach((record) => {
      const key = `${record.key}::${record.sessionId}`;
      const group = sessionGroups.get(key) || [];
      group.push(record);
      sessionGroups.set(key, group);
    });
    const normalizedRecords = [...sessionGroups.values()].map((items) => {
      const automatic = items.filter((item) => item.automatic).sort((a, b) => b.time - a.time)[0];
      const primary = automatic || items[0];
      return {
        ...primary,
        count: automatic ? automatic.count : items.reduce((sum, item) => sum + item.count, 0),
        type: [...new Set(items.map((item) => item.type).filter(Boolean))].join(" | "),
        time: Math.max(...items.map((item) => item.time)),
        recordedAt: items.sort((a, b) => b.time - a.time)[0]?.recordedAt || primary.recordedAt,
      };
    });
