...
(function initKnowledgeMapping(global) {
  ...
  function conceptDescriptor(concept = {}, base = {}) {
    const key = normalize(concept.canonicalKey || concept.canonicalTitle || concept.title);
    const mappings = (base.topicMappings || []).filter((mapping) => (mapping.conceptKeys || [mapping.canonicalKey]).map(normalize).includes(key));
    const evidence = (base.evidence || []).filter((item) => normalize(item.canonicalKey) === key);
    const aliases = (base.aliases || []).filter((alias) => normalize(alias.conceptKey) === key);
  ...
  function subjectScore(target, concept) {
    ...
    const compatible = concept.subjects.some((source) => HISTORY?.subjectCompatibility?.(target.subject, source)?.score >= .45);
  ...
  function candidateScore(target, concept) {
    ...
    const alias = concept.aliases.find((item) => normalize(item.aliasNormalized || item.aliasDisplay) === title);
  ...
  function candidateConcepts(base = {}, target = {}) {
    const descriptor = topicDescriptor(target);
    return (base.concepts || []).map((concept) => ({ concept, descriptor: conceptDescriptor(concept, base), match: candidateScore(descriptor, conceptDescriptor(concept, base)) })).filter((item) => item.match.accepted).sort((a, b) => b.match.score - a.match.score || a.descriptor.key.localeCompare(b.descriptor.key));
  }
  ...
  function matchTopicToConcepts(target = {}, base = {}) {
    const descriptor = topicDescriptor(target);
    const rule = findRule(base, descriptor);
    ...
    const candidates = candidateConcepts(base, descriptor);
    ...
    const conceptKeys = selected.map((item) => item.descriptor.key);
  ...
  function matchConceptToTopics(base = {}, topics = []) { return topics.map((topic) => matchTopicToConcepts(topic, base)).filter((mapping) => mapping.conceptKeys.length); }
  function suggestMappings(base = {}, topics = []) { return (Array.isArray(topics) ? topics : [topics]).map((topic) => matchTopicToConcepts(topic, base)); }
  ...
  function applyMappingDecision(base = {}, mapping = {}, decision = "confirm", now = new Date().toISOString()) {
    ...
    const conceptKeys = unique(mapping.conceptKeys || mapping.confirmedConceptKeys || mapping.suggestedConceptKeys).map(normalize).filter(Boolean);
  ...
  function migrateKnowledgeMappings(base = {}) {
    const mappings = (base.topicMappings || []).map((item) => ({ ...item, conceptKeys: unique(item.conceptKeys || [item.canonicalKey || item.conceptId?.replace(/^concept:/, "")]).map(normalize).filter(Boolean), status: item.status || (item.basis === "user-confirmed" ? "user-confirmed" : "auto-confirmed") }));
    return { ...base, schemaVersion: Math.max(2, Number(base.schemaVersion) || 1), topicMappings: mappings, aliases: Array.isArray(base.aliases) ? base.aliases.map((item) => ({ ...item })) : [], mappingRules: Array.isArray(base.mappingRules) ? base.mappingRules.map((item) => ({ ...item, conceptKeys: unique(item.conceptKeys).map(normalize) })) : [], mappingRejections: Array.isArray(base.mappingRejections) ? base.mappingRejections.map((item) => ({ ...item, conceptKeys: unique(item.conceptKeys).map(normalize) })) : [] };
  }
  ...
  const api = { normalize, topicDescriptor, conceptDescriptor, mappingIdentity, candidateConcepts, matchTopicToConcepts, matchConceptToTopics, suggestMappings, applyMappingDecision, migrateKnowledgeMappings, inspectTopicMapping };
